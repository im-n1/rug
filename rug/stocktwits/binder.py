...
def bind_api(**config):
    class APIMethod(object):
        ...
        rate_limit = config.get('rate_limit', False)
        partner_level = config.get('partner_level', False)
        login_api = config.get('login_api', False)
        rooms_api = config.get('rooms_api', False)
        avatars_api = config.get('avatars_api', False)
        charts_api = config.get('charts_api', False)
        ql_api = config.get('ql_api', False)
        assets_api = config.get('assets_api', False)
        ...
        def build_parameters(self, args, kwargs):
            ...
            for idx, arg in enumerate(args):
                ...
                try:
                    ...
                    raise StocktwitError('Too many parameters supplied!')

            for k, arg in kwargs.items():
                if arg is None:
                    continue
                if k in self.session.params:
                    raise StocktwitError('Multiple values for parameter %s supplied!' % k)
            ...
            for variable in re_path_template.findall(self.path):
                ...
                if name == 'user' and 'user' not in self.session.params and self.api.auth:
                    ...
                    try:
                        ...
                        raise StocktwitError('No parameter value found for path variable: %s' % name)
        ...
        def execute(self):
            ...
            while retries_performed < self.retry_count + 1:
                ...
                try:
                    ...
                    six.reraise(StocktwitError, StocktwitError('Failed to send request: %s' % e), sys.exc_info()[2])
